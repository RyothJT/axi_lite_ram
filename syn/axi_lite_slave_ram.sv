module axi_lite_slave_ram #(
    ...
    parameter ADDR_WIDTH = 8,   // System address bus width
    parameter RAM_DEPTH  = 256  // Must be power of 2 (e.g., 2^ADDR_WIDTH)
) (
    // Global signals
    input logic aclk,
    input logic aresetn,
    ...
    always_ff @(posedge aclk) begin
        if (ram_write_ready) begin
            ...
            $display("Writing %h to %h", write_data_out, ram_write_idx);
    ...
    assign rdata   = ram[read_addr_full[9:2]];
    assign arready = !ar_fifo_full;
    assign rready  = !r_fifo_full;
